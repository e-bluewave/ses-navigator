...
describe('proposal message delivery client', () => {
  it('sends with a fresh idempotency key', async () => {
    ...
    expect(headers.get('idempotency-key')).toBe('delivery-key-1');
  });
  ...
  it('uses a new idempotency key for retry and reads delivery history', async () => {
    ...
    const retryHeaders = new Headers(request.mock.calls[0]?.[1]?.headers);
    expect(retryHeaders.get('idempotency-key')).toBe('retry-key-1');
