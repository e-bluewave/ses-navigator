...
import type {
  ...
  ProposalMessageDeliveryProviderInput,
...
describe('proposal message delivery API', () => {
  it('sends only after preparing an idempotent delivery batch', async () => {
    ...
    const recordResult = vi.fn(() => Promise.resolve(delivery));
    const repo = repository({ prepareSend, recordResult });
    const deliver = vi.fn((input: ProposalMessageDeliveryProviderInput) => {
      void input;
      return Promise.resolve<ProposalMessageDeliveryProviderResult>({
        status: 'accepted',
        providerMessageId: 'fake-message',
        responseCode: '202',
        responsePayload: { accepted: true },
    ...
    const sendProvider: ProposalMessageDeliveryProvider = {
      name: 'fake',
      available: true,
      deliver,
    };
    ...
    expect(delivered?.attemptId).toBe(attemptId);
    expect(delivered?.subject).toBe('提案メール');
    expect(delivered?.recipient.address).toBe('sales@example.com');
  ...
  it('retries only through the retry preparation RPC', async () => {
    const prepareRetry = vi.fn(() => Promise.resolve(preparation));
    const repo = repository({ prepareRetry });
  ...
  it('fails closed when delivery provider is not configured', async () => {
    const prepareSend = vi.fn(() => Promise.resolve(preparation));
    const response = await app(repository({ prepareSend }), {
      name: 'disabled',
      available: false,
      deliver: vi.fn(),
    }).inject({
