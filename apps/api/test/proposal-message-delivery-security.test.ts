...
describe('Proposal message delivery database boundary', () => {
  it(
    'requires both proposal and message send permissions in the current tenant',
    async () => {
  ...
  it(
    'locks approved messages before moving them to queued state',
    async () => {
      const sql = await readFile(migrationUrl, 'utf8');
  ...
  it(
    'retries only failed recipients and preserves append-only attempt numbers',
    async () => {
      const sql = await readFile(migrationUrl, 'utf8');
      ...
      expect(sql).not.toContain("where r.delivery_status in ('failed','bounced')");
      expect(sql).not.toContain('delete from app.message_delivery_attempts');
    },
  );

  it(
    'keeps delivery result recording service-role-only and hides provider payloads from reads',
    async () => {
      const sql = await readFile(migrationUrl, 'utf8');
  ...
  it(
    'keeps direct delivery table writes out of the authenticated API surface',
    async () => {
