...
const ids = Array.from({ length: 20 }, (_, index) => `BA-${String(index + 1).padStart(3, '0')}`);

function makePolicy(statusFor = () => 'pending') {
  const items = ids.map((id) => ({ id, status: statusFor(id), evidence: `${id} evidence` }));
...
test('accepts a structurally valid not-ready policy', () => {
  const policy = makePolicy((id) => (['BA-004', 'BA-018', 'BA-020'].includes(id) ? 'verified' : 'pending'));
...
test('rejects missing and unexpected P0 items', () => {
  ...
  policy.items.push({ id: 'BA-999', status: 'pending', evidence: 'unexpected' });
  const result = evaluateP0ReleaseReadiness(policy);
...
test('rejects declared readiness that does not match computed readiness', () => {
  ...
  const result = evaluateP0ReleaseReadiness(policy);
  assert.ok(result.failures.includes('declared productionReady must match computed readiness'));
});
