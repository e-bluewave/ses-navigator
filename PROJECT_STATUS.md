...
- Migration 121の`row_version`自動加算を適用・回帰確認
- フロントエンド・API実装準備
...
1. Migration 121を空DB・既存DBへ適用し、楽観ロック更新と回帰を確認する
2. フロントエンド・APIの初期構成を確定する
3. 認証・テナント切替・主要業務画面から実装へ進む

# 更新履歴

## 2026-08-04

- Migration 120の複合外部キー用インデックス53件を`Main`へ反映
- `row_version`対象を主要更新テーブル28件に限定し、Migration 121で更新時の自動加算を追加
